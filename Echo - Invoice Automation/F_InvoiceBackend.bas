Attribute VB_Name = "F_InvoiceBackend"
'Version: 1.00
'Ika - Invoice Backend
'For Echobroadband
'User: Ika
...
Sub Invoice_Details(Filename, region)
  filedate = Format(Date, "ddmmyyyy")
  OutputFileName = "Ika-Invoice " & filedate & ".xlsx"
  ...
  'Sheet1 Calculation and pasting
    Call Copytodatasheet1
    Call ProcessingValues(Count1, 4)
    Call CalculateTotal(5, 4 + Count1, Count1)
    Call detailtemplate(0, region, 1)
    Call TableArrangment("G3:BR3")
    Call TableArrangment("C4:BT4")
    If Count1 = 0 Then
        Count1 = 1
    End If
    Call TableArrangmentData("B5:BT" & Count1 + 4)
    Call thickline("AM3:AM" & 5 + Count1)
    Call boardermoney(3, 5 + Count1)
    wo1.Range("G" & 5 + Count1 & ":AL" & 5 + Count1).Copy
    wo2.Range("F14:F45").PasteSpecial Paste:=xlPasteValues, Operation:=xlNone, SkipBlanks:=False, Transpose:=True
    wo1.Range("AM" & 5 + Count1 & ":BR" & 5 + Count1).Copy
    wo2.Range("G14:G45").PasteSpecial Paste:=xlPasteValues, Operation:=xlNone, SkipBlanks:=False, Transpose:=True
    wo2.Range("F46").Value = Application.WorksheetFunction.Sum(wo2.Range("G14:G45"))
  ...
  'Sheet 2 & Sheet 3
    ...
    Call thickline("AM" & 9 + Count1 & ":AM" & 11 + Count1 + Count2 + Count3)
  ...
  'Sheet 4
    ...
    Call thickline("AM" & 15 + Count1 + Count2 + Count3 & ":AM" & 17 + Count1 + Count2 + Count3 + Count4)
    Call boardermoney(15 + Count1 + Count2 + Count3, 17 + Count1 + Count2 + Count3 + Count4)
  ...
  'Sheet 5
    ...
    Call thickline("AM" & Count1 + Count2 + Count3 + Count4 + 21 & ":AM" & 24 + Count1 + Count2 + Count3 + Count5)
...
End Sub
Sub boardermoney(firstrow, lastrow)
    filedate = Format(Date, "ddmmyyyy")
    OutputFileName = "Ika-Invoice " & filedate & ".xlsx"
 ...
 'Invoice detail template
  With wo1
    ...
    With .Range("AN" & lastrow & ":BS" & lastrow).Borders(xlInsideVertical)
        ...
        .Weight = xlThin
    End With
    .Range("AN" & lastrow & ":BR" & lastrow).Borders(xlInsideHorizontal).LineStyle = xlNone
    ...
    End With
    With .Range("BS" & firstrow + 1 & ":BS" & lastrow).Interior
    .Pattern = xlSolid
    .PatternColorIndex = xlAutomatic
    .ThemeColor = xlThemeColorAccent1
    .TintAndShade = 0.799981688894314
    .PatternTintAndShade = 0
    End With
    .Range("AM" & firstrow & ":BT" & firstrow + 1).Font.Bold = True
    .Range("AM" & lastrow & ":BT" & lastrow).Font.Bold = True
 End With
End Sub
Sub thickline(thickline)
  'Data
    filedate = Format(Date, "ddmmyyyy")
    OutputFileName = "Ika-Invoice " & filedate & ".xlsx"
    Outputsheetname1 = "Invoice Details"
...
Sub detailtemplate(x, region, y)
  ...
  'Invoice detail template
   ...
   End With
End Sub
Sub CalculateTotal(StartingValue, EndValue, Count)
    If Count = 0 Then
    ...
    With wsPro
      ...
      With .Range("G" & EndingValue + 1 & ":AL" & EndingValue + 1).Font
        .Color = -10477568
        .TintAndShade = 0
        .Size = 11
        .Bold = True
      ...
      .Rows(EndingValue - Count + x & ":" & EndingValue - Count + x).RowHeight = 18
      Next
      .Rows(EndingValue - Count - 2 & ":" & EndingValue - Count - 2).RowHeight = 34.5
      
   End With
End Sub
Sub TableArrangment(tablesize)
        filedate = Format(Date, "ddmmyyyy")
        OutputFileName = "Ika-Invoice " & filedate & ".xlsx"
    ...
    End With
End Sub
Sub TableArrangmentData(tablesize)
    filedate = Format(Date, "ddmmyyyy")
    OutputFileName = "Ika-Invoice " & filedate & ".xlsx"
    Sheet = "Invoice Details"
    With Workbooks(OutputFileName).Worksheets(Sheet).Range(tablesize).Borders(xlEdgeLeft)
    ...
    End With
End Sub
Sub TableArrangmentDataCalcu(tablesize)
    filedate = Format(Date, "ddmmyyyy")
    OutputFileName = "Ika-Invoice " & filedate & ".xlsx"
    Sheet = "Invoice Details"
    With Workbooks(OutputFileName).Worksheets(Sheet).Range(tablesize).Borders(xlEdgeLeft)
...
Sub Copytodatasheet5()
  ...
  End With
End Sub
Sub ProcessingValues(Count, linespace)
  filedate = Format(Date, "ddmmyyyy")
  OutputFileName = "Ika-Invoice " & filedate & ".xlsx"
  Outputsheet1 = "Invoice Details" 'wo1
  Outputsheet2 = "Invoice Summary" 'wo2
